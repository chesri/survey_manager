...
define([
  ...
  function(declare, array, html, lang, query, on, keys, _WidgetsInTemplateMixin,
    ...
    CustomFeaturelayerChooserFromMap, ToggleButton, SurveyManager, parser, Calendar, date) {

    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      name: 'FilterCalendar',
      baseClass: 'jimu-widget-filter',
      surveyManager: null,
      surveyManagerNeededParameters: null,
      ...
      startup: function() {
        ...
        this.surveyManager = new SurveyManager(this.map, this.config, this.appConfig, this);
      ...
      _initFilters: function(filters, existAskForValue, isCheckLastNode){
        ...
        array.forEach(filters, function(filterObj, idx) {
          ...
          // save the node, filterObj, and parse for use when the calendar month changed
          // assuming there is only one group of filters
          if (this.surveyManagerNeededParameters == null) {
            this.surveyManagerNeededParameters = {};
            this.surveyManagerNeededParameters.node = node;
            this.surveyManagerNeededParameters.filterObj = filterObj;
            this.surveyManagerNeededParameters.parse = parse;
          }
      ...
      toggleFilter: function(node, filterObj, parse) {
        ...
        if (!applied) {
          var dateWhere = this.surveyManager.buildWhereforCurrentMonth();
          layerFilterExpr += " AND (" + dateWhere + ")";
        }
      ...
      _afterFilterApplied: function(layerId){
        this.surveyManager.queryDatesHavingSurveys(layerId);
