...
// jscs:disable
/* jshint ignore:start */
define(["dojo/_base/kernel", "dojo/_base/declare", "dojo/_base/Deferred", "dojo/on", "dojo/has", "dojo/aspect", "dgrid/List", "dojo/has!touch?dgrid/util/touch", "put-selector/put", "dojo/topic", "dojo/query", "dojo/_base/sniff"],
function(kernel, declare, Deferred, on, has, aspect, List, touchUtil, put, topic){
	...
	_handleSelect: function(event, target){
		...
		if(!event.keyCode || !event.ctrlKey || event.keyCode == 32){
			...
			if(!event.shiftKey && event.type === downType && this.isSelected(target)){
				...
				// customization for survey scheduler
				if (target && this.row && this.row(target) && this.row(target).data) {
					topic.publish("survey/tableRowClicked", this.row(target).data);
				}
			}
		}
